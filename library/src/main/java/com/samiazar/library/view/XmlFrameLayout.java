...
public class XmlFrameLayout extends XmlView {


    public XmlFrameLayout(Context context, XmlPullParser xmlParser, ParentType parentType) {
    ...
    public View getView() throws IOException, XmlPullParserException {
        ...
        xmlParser.require(XmlPullParser.START_TAG, TagKey.NameSpace, TagView.FrameLayout);

        // add children of view
        while (xmlParser.next() != XmlPullParser.END_TAG) {
            if (xmlParser.getEventType() != XmlPullParser.START_TAG) continue;
            ((ScrollView) view).addView(XmlParser.getLayout(xmlParser, view.getContext(), ParentType.FRAME_LAYOUT));
        }

        //end of parsing this view
        xmlParser.require(XmlPullParser.END_TAG, TagKey.NameSpace, TagView.FrameLayout);
        return view;
    }
}
